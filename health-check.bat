...
echo.
echo [3/6] Checking User Service (8083)...
powershell -Command "try { $response = Invoke-WebRequest -Uri 'http://localhost:8083/actuator/health' -TimeoutSec 5; Write-Host 'User Service: RUNNING' -ForegroundColor Green } catch { Write-Host 'User Service: NOT RESPONDING (Check database connection)' -ForegroundColor Red }"
...
echo.
echo [6/6] Testing API Gateway Auth Route...
